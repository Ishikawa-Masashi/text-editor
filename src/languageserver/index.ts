// https://github.com/marc2332/codemirror-languageserver/blob/master/src/index.ts
...
class LanguageServerPlugin implements PluginValue {
  ...
  async requestHoverTooltip(
    ...
    let pos = posToOffset(view.state.doc, { line, character }) as any;
    let end!: number;
    if (range) {
      pos = posToOffset(view.state.doc, range.start) as any;
      end = posToOffset(view.state.doc, range.end) as any;
    }
  ...
  processDiagnostics(params: PublishDiagnosticsParams) {
    ...
    const diagnostics = params.diagnostics
      .map(({ range, message, severity }) => ({
        from: posToOffset(this.view.state.doc, range.start) as any,
        to: posToOffset(this.view.state.doc, range.end) as any,
        severity: (
          {
...
export function languageServer(options: LanguageServerWebsocketOptions) {
  ...
  return languageServerWithClient({
    ...
    client: new LanguageServerClient({
      ...options,
      transport: new WebSocketTransport(serverUri as ServerUri),
    }),
  });
}

export function languageServerWithClient(
  options: LanguageServerWithClientOptions
) {
