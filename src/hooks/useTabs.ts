...
export interface TabsUtils {
  ...
  selectTab: (ops: { tab: Tab | null; col: number; row: number }) => void;
  focusTab: (ops: { tab: Tab | null; col: number; row: number }) => void;
  closeTab: (ops: { tab: Tab; col: number; row: number }) => void;
...
export default function useTabs(): TabsUtils {
  ...
  return {
    ...
    openTabs: (newTabs) => {
      const selectedTab = newTabs[0].view_panels[0].tabs.find(
        (tab) => tab.id === newTabs[0].view_panels[0].selected_tab_id
      );
