...
export default class FakeClient extends Emittery implements Client {
  ...
  emitMessage: any; //(message: any) => Promise<void>;
  ...
  // TO-DO
  async write_to_terminal_shell(
    terminal_shell_id: string,
    data: string
  ): Promise<any> {
    if (terminal_shell_id) {
      /* no-op */
    }
    if (data) {
      /* no-op */
  ...
  // TO-DO
  async create_terminal_shell(
    terminal_shell_builder_id: string,
    terminal_shell_id: string
  ): Promise<any> {
    if (terminal_shell_builder_id) {
      /* no-op */
    }
    if (terminal_shell_id) {
      /* no-op */
  ...
  // TO-DO
  async create_language_server(
    language_server_builder_id: string
  ): Promise<any> {
    if (language_server_builder_id) {
      /* no-op */
    }
    return null;
  }
  ...
  // TO-DO
  async write_to_language_server(
