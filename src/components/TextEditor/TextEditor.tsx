...
// import { setSearchQuery, SearchQuery, SearchCursor } from '@codemirror/search';
import { SearchCursor } from '@codemirror/search';
import styled from 'styled-components';
import { SavedState } from '../../tabs/text_editor/text_editor';
...
export default function TextEditor({
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      if (state.find.search) {
        // openSearchPanel(view);
        const matches = new SearchCursor(view.state.doc, state.find.search);
        let selection = { from: 0, to: 0 };
        for (const match of matches) {
          if (state.find.line === view.state.doc.lineAt(match.from).number) {
            selection = match;
            break;
          }
        }
